...
(defclass gestalt-acceptor (hunchentoot:acceptor gestalt-acceptor-mixin) 
  ()
  (:default-initargs
...
(defun gestalt-dispatch-request (acceptor request)
  (let ((*application* (application acceptor)))
	...
	(let ((action-name (intern (subseq (hunchentoot:script-name request) 1))))
	  (if (and (ignore-errors (symbol-function action-name))
		   (get action-name :action-p))
	      (progn
		;; We found the action, execute it
		(funcall (unserialize-action action-name (decode-string (hunchentoot:get-parameter "_a"))))
